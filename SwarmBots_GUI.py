...
class App:
    def __init__(self, window, video_source1, video_source2, video_source3, video_source4, video_source5):
        self.window = window
        self.bit = self.window.iconbitmap('Cam.ico')
        self.window.title("SwarmBots")
        self.window.geometry('1095x625')
        self.window.config(bg = 'Black')
        self.window.resizable(0,0)

        image1 = Image.open("LogoTecMM.png")
        test = ImageTk.PhotoImage(image1)
        label1 = tkinter.Label(image=test)
        label1.image = test
        label1.place(x=730, y=315)
        ...
        #Abrir video
        self.video1 = MyVideoCapture1(self.video_source1)
        self.video2 = MyVideoCapture2(self.video_source2)
        self.video3 = MyVideoCapture3(self.video_source3)
        self.video4 = MyVideoCapture4(self.video_source4)
        self.video5 = MyVideoCapture5(self.video_source5)
    ...
    def update(self):
        ...
        ret1, frame1 = self.video1.get_frame
        ret2, frame2 = self.video2.get_frame
        ret3, frame3 = self.video3.get_frame
        ret4, frame4 = self.video4.get_frame
        ret5, frame5 = self.video5.get_frame
...
class MyVideoCapture1:
    def __init__(self, video_source1):
        ...
        self.video1 = cv2.VideoCapture(video_source1)

        if not self.video1.isOpened():
            raise ValueError("No se puede abrir la fuente de video", video_source1)
...
class MyVideoCapture2:
    def __init__(self, video_source2):
        #Abra la fuente de video
        self.video2 = cv2.VideoCapture(video_source2)

        if not self.video2.isOpened():
            raise ValueError("No se puede abrir la fuente de video", video_source2)
...
# Video Camara 3
class MyVideoCapture3:
    def __init__(self, video_source3):
        #Abra la fuente de video
        self.video3 = cv2.VideoCapture(video_source3)

        if not self.video3.isOpened():
            raise ValueError("No se puede abrir la fuente de video", video_source3)

    @property
    def get_frame(self):
        ret3 = ""
        if self.video3.isOpened():
            ret3, frame3 = self.video3.read()
            frame3 = deteccion_facilal3(frame3)
            frame3 = cv2.resize(frame3, (360, 310))
            if ret3:
                #Devuelve un indicador de éxito booleano y el fotograma actual convertido a BGR
                return ret3, cv2.cvtColor(frame3, cv2.COLOR_BGR2RGB)
            else:
                return ret3
        else:
            return ret3, None
...
# Video Camara 4
class MyVideoCapture4:
    def __init__(self, video_source4):
        #Abra la fuente de video
        self.video4 = cv2.VideoCapture(video_source4)

        if not self.video4.isOpened():
            raise ValueError("No se puede abrir la fuente de video", video_source4)

    @property
    def get_frame(self):
        ret4 = ""
        if self.video4.isOpened():
            ret4, frame4 = self.video4.read()
            frame4 = deteccion_facilal4(frame4)
            frame4 = cv2.resize(frame4, (360, 310))
            if ret4:
                #Devuelve un indicador de éxito booleano y el fotograma actual convertido a BGR
                return ret4, cv2.cvtColor(frame4, cv2.COLOR_BGR2RGB)
            else:
                return ret4
        else:
            return ret4, None
...
# Video Camara 5
class MyVideoCapture5:
    def __init__(self, video_source5):
        #Abra la fuente de video
        self.video5 = cv2.VideoCapture(video_source5)

        if not self.video5.isOpened():
            raise ValueError("No se puede abrir la fuente de video", video_source5)

    @property
    def get_frame(self):
        ret5 = ""
        if self.video5.isOpened():
            ret5, frame5 = self.video5.read()
            frame5 = deteccion_facilal5(frame5)
            frame5 = cv2.resize(frame5, (360, 310))
            if ret5:
                #Devuelve un indicador de éxito booleano y el fotograma actual convertido a BGR
                return ret5, cv2.cvtColor(frame5, cv2.COLOR_BGR2RGB)
            else:
                return ret5
        else:
            return ret5, None
...
initial = tkinter.Tk()
bit = initial.iconbitmap('Cam.ico')
...
L9 = tkinter.Label(initial, text="Welcome")
